...
function Effect(this: any, cfg: any) {
  ...
  this.timer_o;
  this.active = false;
  this.use = function (y: any, z: any) { };
  this.un = function (x: any, y: any, z: any) { };
  this.mods = function () { }
  this.onGive = function () { };
  this.onRemove = function (x: any) { };
...
effect.strawp = new Effect({ type: 2,
  use: function () { you.satmax += 50; you.sat += 50 },
  un: function () { you.sat -= 50 },
...
effect.psnwrd = new Effect({ id: 3, name: 'Poison block', desc: 'Weak poisons have no effect on you', type: 3, duration: 600, x: '＋', c: 'lime', b: 'darkmagenta',
  onGive: function () { msg('You feel safer', 'lime') },
  use: function () {
...
effect.psnwrd2 = new Effect({ id: 4, name: 'Venom block', desc: 'Severe poisons have no effect on you', type: 3, duration: 600, x: '＋', c: 'lime', b: 'magenta',
  onGive: function () { msg('You feel much safer', 'lime') },
  use: function () {
    if (--this.duration === 0) { removeEff(this); this.duration = 600; };
  }
});
...
effect.fpn = new Effect({ id: 8, name: 'Food poisoning', desc: 'From eating something bad', type: 3, duration: 30, x: '«', c: 'lime', b: 'grey',
  onGive: function () { msg(select(['You feel bad inside', 'Your stomach bothers you']), 'green') },
  use: function (y: any, z: any) {
...
effect.wet = new Effect({ id: 9, name: 'Wet', desc: 'You\'re drenched in water', type: 3, duration: 5, x: '雨', c: 'cyan', b: 'blue',
  onGive: function () { if (this.target.id === you.id) { msg('Your clothes get soaked', 'cyan', null, null, 'blue'); global.flags.iswet = true } },
  onRemove: function () { msg('You dry up', 'orange'); global.flags.iswet = false },
  use: function () {
...
effect.fplc = new Effect({ id: 10, save: false, name: 'Fireplace Aura', desc: 'You\'re feeling the warmth of the fireplace', type: 3, duration: 2, x: '火', c: 'yellow', b: 'crimson',
  onGive: function () { you.mods.ckfre += 1; },
  use: function () {
  ...
  onRemove: function () { you.mods.ckfre -= 1; }
});
...
effect.cdlt = new Effect({ id: 11, name: 'Candlelight', desc: 'You\'re carrying a candle. The surroundings are lit up', type: 3, duration: 360, x: '❛', c: 'gold', b: '#440205',
  ...
  onGive: function () { you.mods.light += 1; },
  onRemove: function () { you.mods.light -= 1; }
});
...
effect.tarnish = new Effect({ id: 15, name: 'Tarnished', desc: 'Equipment usability -30%', type: 4, duration: -1, x: '≠', c: 'purple', b: 'grey',
  onGive: function () { msg('Your equipment cracks', 'purple') },
  use: function (y: any, z: any) {
  }
});
...
effect.prostasia = new Effect({ id: 16, name: 'Prostasía', desc: 'Equipment usability +30%', type: 4, duration: -1, x: '≒', c: 'midnightblue', b: 'skyblue',
  onGive: function () { msg('You feel secure', 'skyblue') },
  use: function (y: any, z: any) {
  }
});
...
effect.drunk = new Effect({ id: 19, name: 'Inebriated', desc: 'You\'re feeling drunk from alcohol', type: 5, duration: 15, x: '酒', c: 'darkred', b: 'orange',
  ...
  mods: function () { you.agle /= 1 + (.4 - skl.drka.lvl * .03); you.stre *= 1 + (.2 + skl.drka.lvl * .02); you.inte /= 1 + (.5 - skl.drka.lvl * .04) },
  onGive: function () { msg('You\'re feeling tipsy', 'chocolate') },
  onRemove: function () { msg('You sober up', 'orange') }
});
...
effect.virus = new Effect({ id: 20, name: 'Virus', desc: 'You are contaminated', type: 5, duration: -1, x: '⁑', c: 'black', b: 'lightgrey',
  ...
  mods: function () { you.agle /= 1.1; you.stre /= 1.1; you.sat -= 70; you.sata -= 70 },
  onGive: function () { msg('You feel bad', 'grey') },
  onRemove: function () { msg('You feel better', 'orange') }
});
...
effect.invgrt = new Effect({ id: 22, name: 'Invigorate', desc: 'Your joints feel flexible', type: 3, duration: -1, x: 'ℐ', c: 'yellowgreen', b: 'darkgreen',
  onGive: function () { if (!this.active) { msg(this.target.id === you.id ? 'You become nimble' : (this.target.name + ' becomes nimble'), 'green'); this.target.aglm += .3 } },
  onRemove: function () { this.target.aglm -= .3 },
  use: function () {
    if (--this.duration === 0) {
...
effect.cold = new Effect({ id: 24, name: 'Cold', desc: 'You\'re freezing', type: 5, duration: 5, x: '冷', c: '#88a', b: '#eef',
  mods: function () { you.agle /= 1.1; you.stre /= 1.1; you.hpe /= 1.1; you.sate /= 1.05 },
  onGive: function () { if (this.target.id === you.id) msg('You feel colder', 'blue', null, null, 'cyan'); },
  onRemove: function () { if (this.target.id === you.id) msg('You\'re warming up', 'orange'); },
  use: function () {
    if (this.target.id === you.id) {
...
effect.smoke = new Effect({ id: 25, name: 'Smoke', desc: 'Thick smoke abstructs your lungs', type: 3, duration: 5, x: '煙', c: 'grey', b: 'lightgrey',
  onGive: function () { if (this.target.id === you.id) { msg('You breathe heavily', 'grey') } },
  onRemove: function () { msg('Your lungs feel lighter', 'orange') },
  use: function () {
    if (this.target.id === you.id) {
      if (random() < .1) {
...
effect.fbite = new Effect({ id: 26, name: 'Hypothermia', desc: 'Your limbs are suffering from frostbites', type: 5, duration: 5, x: '凍', c: 'red', b: '#aaf',
  mods: function () { you.agle /= 1.15; you.stre /= 1.2; you.hpe /= 1.2; you.sate /= 1.1 },
  onGive: function () { if (this.target.id === you.id) msg('Sharp pain stings you', 'red', null, null, 'cyan') },
  onRemove: function () { if (this.target.id === you.id) { msg('You aren\'t freezing anymore', 'orange'); global.stat.coldnt = 0 } },
  use: function () {
    if (this.target.id === you.id) {
